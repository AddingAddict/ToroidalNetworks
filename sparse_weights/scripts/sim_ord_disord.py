...
parser.add_argument('--b_idx', '-b',  help='which baseline', type=int, default=0)
...
def simulate_ord_networks(prms,rX,cA):
    ...
    net,this_M,this_H,this_B,this_LAS = su.gen_ring_order_tensor(prms)
    M = this_M.cpu().numpy()
    ...
    base_sol,_ = integ.sim_dyn_tensor(ri,T,0.0,this_M,rX*(this_B+cA*this_H),
                                      this_LAS,net.C_conds[0],mult_tau=True,max_min=30)
    rs[0] = np.mean(base_sol[:,mask_time].cpu().numpy(),-1)
    ...
    opto_sol,_ = integ.sim_dyn_tensor(ri,T,1.0,this_M,rX*(this_B+cA*this_H),
                                      this_LAS,net.C_conds[0],mult_tau=True,max_min=30)
    rs[1] = np.mean(opto_sol[:,mask_time].cpu().numpy(),-1)
    ...
    muXs[0] = H
    muEs[0] = M[:,net.C_all[0]]@rs[0,net.C_all[0]] + H
    muIs[0] = M[:,net.C_all[1]]@rs[0,net.C_all[1]]
    muXs[0,net.C_all[0]] *= ri.tE
    muXs[0,net.C_all[1]] *= ri.tI
    muEs[0,net.C_all[0]] *= ri.tE
    muEs[0,net.C_all[1]] *= ri.tI
    muIs[0,net.C_all[0]] *= ri.tE
    muIs[0,net.C_all[1]] *= ri.tI
    mus[0] = muEs[0] + muIs[0]

    muXs[1] = H
    muEs[1] = M[:,net.C_all[0]]@rs[1,net.C_all[0]] + H
    muIs[1] = M[:,net.C_all[1]]@rs[1,net.C_all[1]]
    muXs[1,net.C_all[0]] *= ri.tE
    muXs[1,net.C_all[1]] *= ri.tI
    muEs[1,net.C_all[0]] *= ri.tE
    muEs[1,net.C_all[1]] *= ri.tI
    muIs[1,net.C_all[0]] *= ri.tE
    muIs[1,net.C_all[1]] *= ri.tI
    muXs[1] = muXs[1] + LAS
    muEs[1] = muEs[1] + LAS
    mus[1] = muEs[1] + muIs[1]
    ...
    return net,rs,mus,muXs,muEs,muIs

def simulate_disord_networks(prms,rX,cA):
    ...
    for seed_idx,seed in enumerate(seeds):
        ...
        net,this_M,this_H,this_B,this_LAS,this_EPS = su.gen_ring_disorder_tensor(seed,prms,0)
        M = this_M.cpu().numpy()
...
print("Simulating ordered network")
print('')
        
ordrEs = np.zeros((3,Nori))
ordrIs = np.zeros((3,Nori))
ordhEs = np.zeros((3,Nori))
ordhIs = np.zeros((3,Nori))
ordbalEs = np.zeros((2,Nori))
ordbalIs = np.zeros((2,Nori))

net,rs,mus,muXs,muEs,muIs = simulate_ord_networks(prms,rX,cA)

for nloc in range(Nori):
    ordrEs[:2,nloc] = np.mean(rs[:,net.C_idxs[0][nloc]],axis=-1)
    ordrIs[:2,nloc] = np.mean(rs[:,net.C_idxs[1][nloc]],axis=-1)
    ordhEs[:2,nloc] = np.mean(mus[:,net.C_idxs[0][nloc]],axis=-1)
    ordhIs[:2,nloc] = np.mean(mus[:,net.C_idxs[1][nloc]],axis=-1)
    ordbalEs[:,nloc] = np.mean(np.abs(mus[:,net.C_idxs[0][nloc]])/muEs[:,net.C_idxs[0][nloc]],axis=-1)
    ordbalIs[:,nloc] = np.mean(np.abs(mus[:,net.C_idxs[1][nloc]])/muEs[:,net.C_idxs[1][nloc]],axis=-1)

    ordrEs[2,nloc] = np.mean(rs[1,net.C_idxs[0][nloc]]-rs[0,net.C_idxs[0][nloc]],axis=-1)
    ordrIs[2,nloc] = np.mean(rs[1,net.C_idxs[1][nloc]]-rs[0,net.C_idxs[1][nloc]],axis=-1)
    ordhEs[2,nloc] = np.mean(mus[1,net.C_idxs[0][nloc]]-mus[0,net.C_idxs[0][nloc]],axis=-1)
    ordhIs[2,nloc] = np.mean(mus[1,net.C_idxs[1][nloc]]-mus[0,net.C_idxs[1][nloc]],axis=-1)

# Simulate network where structure is removed by increasing baseline fraction
print('simulating disordered networks')
print('')
...
net,rs,mus,muXs,muEs,muIs,Ls,TOs = simulate_disord_networks(prms,rX,cA)
...
print("Simulating disordered networks and computing statistics took ",time.process_time() - start," s")
print('')
...
res_file = res_file + '_b_{:d}_c_{:d}'.format(b_idx, c_idx)
