...
for cA_idx,cA in enumerate(cAs):
    ...
    for nloc in range(Nori):
        ...
        μhIs[cA_idx,nloc] = np.mean(mus[:,net.C_idxs[1][nloc],:])
        ΣhEs[cA_idx,nloc] = np.var(mus[:,net.C_idxs[0][nloc],:])
        ΣhIs[cA_idx,nloc] = np.var(mus[:,net.C_idxs[1][nloc],:])
        balEs[cA_idx,nloc] = np.mean(np.abs(mus[:,net.C_idxs[0][nloc],:])/muEs[:,net.C_idxs[0][nloc],:])
        balIs[cA_idx,nloc] = np.mean(np.abs(mus[:,net.C_idxs[1][nloc],:])/muEs[:,net.C_idxs[1][nloc],:])
    Lexps[cA_idx] = np.mean(Ls)
