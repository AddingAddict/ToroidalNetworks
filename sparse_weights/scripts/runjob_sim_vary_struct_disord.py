...
def runjobs():
    ...
    for s in s_Vec:
        for d in d_Vec:
            ...
            #--------------------------------------------------------------------------
            # Make SBTACH
            inpath = currwd + "/sim_vary_struct_disord.py"
            c1 = "{:s} -s {:d} -d {:d}".format(inpath,s,d)
            
            jobname="sim_vary_struct_disord"+"-s-{:d}-d-{:d}".format(s,d)
            ...
            if not args2.test:
                ...
                text_file.write("#SBATCH --mem-per-cpu=60gb \n")
