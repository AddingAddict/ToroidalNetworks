...
def runjobs():
    ...
    for r in r_Vec:
        for k in k_Vec:
            ...
            #--------------------------------------------------------------------------
            # Make SBTACH
            inpath = currwd + "/sim_vary_rX_K.py"
            c1 = "{:s} -r {:d} -k {:d}".format(inpath,r,k)
            
            jobname="sim_vary_rX_K"+"-r-{:d}-k-{:d}".format(r,k)
            ...
            if not args2.test:
                ...
                text_file.write("#SBATCH --mem-per-cpu=20gb \n")
    ...
    for r in r_Vec:
        for n in n_Vec:
            ...
            if not args2.test:
                ...
                text_file.write("#SBATCH --job-name="+jobname+ "\n")
                text_file.write("#SBATCH -t 0-11:59  \n")
                text_file.write("#SBATCH --mem-per-cpu=16gb \n")
                text_file.write("#SBATCH --gres=gpu\n")
                text_file.write("#SBATCH -c 1 \n")
                text_file.write("#SBATCH -o "+ ofilesdir + "/%x.%j.o # STDOUT \n")
                text_file.write("#SBATCH -e "+ ofilesdir +"/%x.%j.e # STDERR \n")
