...
def find_rX_to_fix_r(prms,rX0,cA,CVh):
    ...
    for rX_idx,rX in enumerate(rXs):
        ...
        for seed_idx,seed in enumerate(fix_r_seeds):
            ...
            vsm_rs[rX_idx,seed_idx] = np.mean(np.mean(sol[:,mask_time].cpu().numpy(),-1)[net.get_oriented_neurons()])

            print("Integrating base network took ",time.process_time() - start," s")
            print('this vsm_rs =',vsm_rs[rX_idx,seed_idx])
            print('')
            
    vsm_rs = np.mean(vsm_rs,-1)

    vsm_rs_itp = RegularGridInterpolator((rXs,), vsm_rs)
    
    def pred_vsm_rs(x):
        try:
            return vsm_rs_itp(x[:,None])
        except:
            return vsm_rs_itp([x])
    
    def residuals(x):
        pred_vsm_r = pred_vsm_rs(x)
        res = np.array([r_target - pred_vsm_r])
        return res.ravel()
    ...
    print('predicted vsm_r =',pred_vsm_rs(results.x[0]))
