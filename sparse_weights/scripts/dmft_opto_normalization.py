...
def predict_networks(prms,rX,cA,CVh,dori=45):    
    ...
    Cdrpmb = (Cdrp - Cdrb)*np.sum(dmft.inv_overlap(xpeaks,sCdr[:,None])[:,:,0],-1)
    ...
    Sigdp = Sigdb + (dmft.struct_fact(0,sWCdr,sCdr)+dmft.struct_fact(dori,sWCdr,sCdr))*SigW*Cdrpmb
    Sigdb = Sigdb + 2*dmft.struct_fact(90,sWCdr,sCdr)*SigW*Cdrpmb
    sSigd = sWCdr
    ...
    for i in range(2):
        ...
        μmuEs[i,0] = gauss(oris,muvb[i,0],muvp[i,0],smuv[i,0]) +\
            gauss_naive(oris,muHb[i],muHp[i],smuH[i])
        μmuEs[i,1] = gauss(oris,muob[i,0],muop[i,0],smuo[i,0]) +\
            gauss_naive(oris,muHb[i],muHp[i],smuH[i]) + prms['L']*1e-3
        μmuEs[i,2] = μmuEs[i,1] - μmuEs[i,0]
        ΣmuEs[i,0] = gauss(oris,Sigvb[i,0],Sigvp[i,0],sSigv[i,0]) +\
            (gauss_naive(oris,muHb[i],muHp[i],smuH[i])*eH)**2
