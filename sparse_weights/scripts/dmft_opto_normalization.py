...
def predict_networks(prms,rX,cA,CVh,dori=45):    
    ...
    if cA == 0 or prms.get('basefrac',0)==1:
        ...
        res_dict = dmft.run_2feat_ring_dmft(prms,rX,cA,CVh,'./../results',ri,Twrm,Tsav,dt)
        rvb = res_dict['rb'][:2]
        rvp = res_dict['rp'][:2]
        srv = res_dict['sr'][:2]
        rob = res_dict['rb'][2:]
    ...
    for i in range(2):
        ...
        μmuEs[i,0] = gauss(oris,muvb[i,0],muvp[i,0],smuv[i,0]*np.ones((1))) +\
            gauss_naive(oris,muHb[i],muHp[i],smuH[i]*np.ones((1)))
        μmuEs[i,1] = gauss(oris,muob[i,0],muop[i,0],smuo[i,0]*np.ones((1))) +\
            gauss_naive(oris,muHb[i],muHp[i],smuH[i]*np.ones((1))) + prms['L']*1e-3
        ΣmuEs[i,0] = gauss(oris,Sigvb[i,0],Sigvp[i,0],sSigv[i,0]*np.ones((1))) +\
            (gauss_naive(oris,muHb[i],muHp[i],smuH[i]*np.ones((1)))*eH)**2
