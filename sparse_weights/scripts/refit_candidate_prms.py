...
for aX_idx,aX in enumerate(aXs):
    ...
    for bX_idx,bX in enumerate(bXs):
        ...
        for seed_idx,seed in enumerate(seeds):
            ...
            vsm_base_means[aX_idx,bX_idx,seed_idx] = np.mean(base_rates[net.get_oriented_neurons()])
            vsm_base_stds[aX_idx,bX_idx,seed_idx] = np.std(base_rates[net.get_oriented_neurons()])
            vsm_opto_means[aX_idx,bX_idx,seed_idx] = np.mean(opto_rates[net.get_oriented_neurons()])
            vsm_opto_stds[aX_idx,bX_idx,seed_idx] = np.std(opto_rates[net.get_oriented_neurons()])
            vsm_diff_means[aX_idx,bX_idx,seed_idx] = np.mean(diff_rates[net.get_oriented_neurons()])
            vsm_diff_stds[aX_idx,bX_idx,seed_idx] = np.std(diff_rates[net.get_oriented_neurons()])
            vsm_norm_covs[aX_idx,bX_idx,seed_idx] = np.cov(base_rates[net.get_oriented_neurons()],
                diff_rates[net.get_oriented_neurons()])[0,1] / vsm_diff_stds[aX_idx,bX_idx,seed_idx]**2
...
with open('./../results/refit_candidate_prms_{:d}'.format(njob)+'.pkl',
          'wb') as handle:
