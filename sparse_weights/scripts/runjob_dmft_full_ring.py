...
def runjobs():
    ...
    for c in c_Vec:
        ...
        #--------------------------------------------------------------------------
        # Make SBTACH
        inpath = currwd + "/dmft_full_ring.py"
        ...
        if np.isclose(SoriE_mult,1.0) and np.isclose(SoriI_mult,1.0) and\
            ...
            jobname="dmft_full_ring"+"-c-{:d}".format(c)
        else:
            jobname="dmft_full_ring"+"-SoriEx{:.2f}-SoriIx{:.2f}-SoriFx{:.2f}-CVhx{:.2f}-Jx{:.2f}-betax{:.2f}-gEx{:.2f}-gIx{:.2f}-hEx{:.2f}-hIx{:.2f}-Lx{:.2f}-CVLx{:.2f}-c-{:d}".format(
