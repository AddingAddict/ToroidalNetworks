...
aXs = np.linspace(0,1.5*fit_aXs[-1],11)
bXs = np.linspace(0.5*fit_bX,1.5*fit_bX,7)
eXs = np.linspace(0.5*fit_eX,1.5*fit_eX,7)
...
def gen_prms(seed):
    ...
    prm_dict['K'] = 500
    prm_dict['SoriE'] = rng.uniform(0.75,1.25)*fit_prms['SoriE']
    prm_dict['SoriI'] = rng.uniform(0.75,1.25)*fit_prms['SoriE']
    prm_dict['SoriF'] = fit_prms['SoriF']
    prm_dict['J'] = rng.uniform(0.75,1.25)*fit_prms['J']
    prm_dict['beta'] = rng.uniform(0.75,1.25)*fit_prms['beta']
    prm_dict['gE'] = rng.uniform(0.75,1.25)*fit_prms['gE']
    prm_dict['gI'] = rng.uniform(0.75,1.25)*fit_prms['gI']
    prm_dict['hE'] = rng.uniform(0.75,1.25)*fit_prms['hE']
    prm_dict['hI'] = rng.uniform(0.75,1.25)*fit_prms['hI']
    prm_dict['L'] = rng.uniform(0.75,1.25)*fit_prms['L']
    prm_dict['CVL'] = rng.uniform(0.75,1.25)*fit_prms['CVL']
...
max_nrep = 10000
for idx_rep in range(first_rep,max_nrep):
    print('repetition number ',idx_rep)
    print('')

    start = time.process_time()

    seed = (ntry*max_nrep+idx_rep)%2**32
    ...
    def fit_best_mous_inputs(eX):
        ...
        x0 = np.concatenate(([fit_bX],np.linspace(fit_aXs[1],fit_aXs[-1],mous_nc-1)))
