...
def predict_ord_networks(prms,rX,cA):
    ...
    rs = np.zeros((2,3,Nori))
    muEs = np.zeros((2,3,Nori))
    muIs = np.zeros((2,3,Nori))
    
    oris = np.arange(Nori)*180/Nori
    oris[oris > 90] = 180 - oris[oris > 90]
        
    def gauss(x,b,p,s):
        return b + (p-b)*mft.basesubwrapnorm(x,s)
    
    if cA == 0 or prms.get('basefrac',0)==1:
        res_dict = mft.run_first_stage_mft(prms,rX*(1+cA),'./../results',ri,Twrm,Tsav,dt)
        rvb = res_dict['r'][:2]
        rvp = res_dict['r'][:2]
        srv = 1e2*np.ones(2)
        rob = res_dict['r'][2:]
        rop = res_dict['r'][2:]
        sro = 1e2*np.ones(2)
        mft_res = res_dict.copy()
    else:
        res_dict = mft.run_first_stage_ring_mft(prms,rX,cA,'./../results',ri,Twrm,Tsav,dt)
        rvb = res_dict['rb'][:2]
        rvp = res_dict['rp'][:2]
        srv = res_dict['sr'][:2]
        rob = res_dict['rb'][2:]
    ...
    muvb = (muW+mft.unstruct_fact(srv)*muWb)*rvb
    muvp = muvb + mft.struct_fact(0,sWrv,srv)*muW*(rvp-rvb)
    muvb = muvb + mft.struct_fact(90,sWrv,srv)*muW*(rvp-rvb)
    smuv = sWrv
    muob = (muW+mft.unstruct_fact(sro)*muWb)*rob + prms['L']*np.array([1,0])
    muop = muob + mft.struct_fact(0,sWro,sro)*muW*(rop-rob)
    muob = muob + mft.struct_fact(90,sWro,sro)*muW*(rop-rob)
    smuo = sWro
    
    for i in range(2):
        rs[i,0] = gauss(oris,rvb[i],rvp[i],srv[i])
        rs[i,1] = gauss(oris,rob[i],rop[i],sro[i])
        rs[i,2] = rs[i,1] - rs[i,0]
        muEs[i,0] = gauss(oris,muvb[i,0],muvp[i,0],smuv[i,0]) + gauss(oris,muHb[i],muHp[i],smuH[i])
        muEs[i,1] = gauss(oris,muob[i,0],muop[i,0],smuo[i,0]) + gauss(oris,muHb[i],muHp[i],smuH[i])
        muEs[i,2] = muEs[i,1] - muEs[i,0]
        muIs[i,0] = gauss(oris,muvb[i,1],muvp[i,1],smuv[i,1])
        muIs[i,1] = gauss(oris,muob[i,1],muop[i,1],smuo[i,1])
        muIs[i,2] = muIs[i,1] - muIs[i,0]
    mus = muEs + muIs

    return rs,mus,muEs,muIs,mft_res

def predict_disord_networks(prms,rX,cA):
    tau = np.array([ri.tE,ri.tI],dtype=np.float32)
    W = prms['J']*np.array([[1,-prms['gE']],[1./prms['beta'],-prms['gI']/prms['beta']]],dtype=np.float32)
    Ks = (1-prms.get('basefrac',0))*np.array([prms['K'],prms['K']/4],dtype=np.float32)
    Kbs =   prms.get('basefrac',0) *np.array([prms['K'],prms['K']/4],dtype=np.float32)
    ...
    if cA == 0 or prms.get('basefrac',0)==1:
        res_dict = dmft.run_two_stage_dmft(prms,rX*(1+cA),0,'./../results',ri,Twrm,Tsav,dt)
        rvb = res_dict['r'][:2]
        rvp = res_dict['r'][:2]
        srv = 1e2*np.ones(2)
        rob = res_dict['r'][2:]
        ...
        res_dict = dmft.run_two_stage_ring_dmft(prms,rX,cA,0,'./../results',ri,Twrm,Tsav,dt)
        rvb = res_dict['rb'][:2]
        rvp = res_dict['rp'][:2]
        srv = res_dict['sr'][:2]
        rob = res_dict['rb'][2:]
    ...
    for i in range(2):
        ...
        ΣmuEs[i,0] = gauss(oris,Sigvb[i,0],Sigvp[i,0],sSigv[i,0])
        ΣmuEs[i,1] = gauss(oris,Sigob[i,0],Sigop[i,0],sSigo[i,0])
...
print('computing ordered network')
print('')
...
rs,mus,muEs,muIs,dmft_res = predict_ord_networks(prms,rX,cA)
...
for nloc in range(Nori):
    ordbalEs[0,nloc] = calc_bal(muEs[0,0,nloc],muIs[0,0,nloc],0,0,N=1)
    ordbalIs[0,nloc] = calc_bal(muEs[1,0,nloc],muIs[1,0,nloc],0,0,N=1)
    ordbalEs[1,nloc] = calc_bal(muEs[0,1,nloc],muIs[0,1,nloc],0,0,N=1)
    ordbalIs[1,nloc] = calc_bal(muEs[1,1,nloc],muIs[1,1,nloc],0,0,N=1)

print("Saving statistics took ",time.process_time() - start," s")
print('')

print('computing disordered network')
print('')
...
μrs,Σrs,μmus,Σmus,μmuEs,ΣmuEs,μmuIs,ΣmuIs,normC,conv,dmft_res = predict_disord_networks(prms,rX,cA)
...
res_file = './../results/dmft_ord_disord_{:s}'.format(str(id))
